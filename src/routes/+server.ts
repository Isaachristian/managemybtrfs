...
export const GET = (({ url }) => {
  ...
  const freeSpace = execSync(`sudo btrfs filesystem usage -b ${mountPoint} | grep "Free (estimated)"`)
          .toString()
          .split(/[ \n\t\(\)]/)
          .map(s => s.trim())
  ...
  const ratio = execSync(`sudo btrfs filesystem usage -b ${mountPoint} | grep "Data ratio"`)
          ...
          .split(/[ \n\t\(\)]/)
          .map(s => s.trim())
          .filter(s => !ignore.includes(s)).at(0)
  const usage = execSync(`sudo btrfs filesystem usage -b ${mountPoint} | grep "Used:"`)
          .toString().split(/[ \n\t\(\)]/).map(s => s.trim()).filter(s => !ignore.includes(s)).at(0)
  const size = execSync(`sudo btrfs filesystem usage -b ${mountPoint} | grep "Device size:"`)
          .toString().split(/[ \n\t\(\)]/).map(s => s.trim()).filter(s => !ignore.includes(s)).at(0)
