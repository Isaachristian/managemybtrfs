<script lang="ts">
  ...
  import {faHouse, faCog, faGaugeHigh, faFolderTree} from '@fortawesome/free-solid-svg-icons'
...
<div class="absolute inset-0 text-white">
  <div class="relative w-full h-full bg-gray-900">
    <!-- Left Nav -->
    <div class="absolute left-2 top-2 bottom-2 w-16 p-2 pb-0 bg-gray-800 rounded-xl">
      <nav class="relative w-full h-full">
        <NavItem href={'/'} icon={faHouse}/>
        <NavItem href={'/Statistics'} icon={faGaugeHigh}/>
        <NavItem href={'/Statistics'} icon={faFolderTree}/>
